...
type Server struct {
	handler query_orders.QueryOrdersServer
	cfg     *config.Config
	srv     *grpc.Server
}
...
func (s *Server) Start() error {
	...
	query_orders.RegisterQueryOrdersServer(s.srv, s.handler)
	go s.srv.Serve(lis)
...
func NewServer(h query_orders.QueryOrdersServer, lc fx.Lifecycle, cfg *config.Config, lg *logging.ZapLogger) *Server {
