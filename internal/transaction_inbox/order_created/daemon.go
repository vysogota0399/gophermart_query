...
type InboxEventsRepository interface {
	ReserveOrderCreatedEvent(ctx context.Context) (*models.OrderCreatedEvent, error)
	SetState(ctx context.Context, uuid string, newState string) error
}

type OrdersRepository interface {
	Create(ctx context.Context, in *models.Order) error
}
...
func (dmn *Daemon) Start() {
	...
	dmn.globalCtx = dmn.lg.WithContextFields(ctx, zap.String("name", "order_created_events_daemon"))
...
func (dmn *Daemon) processEvent(ctx context.Context) error {
	e, err := dmn.events.ReserveOrderCreatedEvent(ctx)
	if err != nil {
	...
	if err := dmn.orders.Create(
		ctx,
		&models.Order{
			UUID:       e.Meta.UUID,
			Number:     e.Meta.Number,
			State:      e.Meta.State,
			UploadedAt: e.Meta.UploadedAt,
